...
R3BNeulandNeutronReconstruction::R3BNeulandNeutronReconstruction(const TString input, const TString output)
    ...
    , fEngine(new Neuland::ReconstructionEngine())
    , fClusters(input)
    , fNeutrons(output)
{
    // Default Operations
    fEngine->AddOperation(std::unique_ptr<Neuland::ReconstructionOperation>(new Neuland::Multiplicity2DCalibr()));
    fEngine->AddOperation(std::unique_ptr<Neuland::ReconstructionOperation>(new Neuland::GetNeutrons()));
